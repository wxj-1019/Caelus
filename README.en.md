...
[![Self-tests](https://img.shields.io/badge/self%2Dtests-295%20passing-%233DD68C)](https://github.com/wxj-1019/Caelus)
...
- Scene stats: focus time, compile sessions and daily-family uptime are kept per day locally (60 days) with 7-day trend charts on both scenario pages, a daily focus goal progress line, and a next-maintenance countdown in the maintenance center; distraction hits are counted per app with an optional focus block (off by default)
- Live monitor page: real-time scenario status tiles, currently suppressed and boosted process lists (tier, reason, time under suppression) and a 30-entry action feed, refreshing every 2 seconds
...
The built-in suite currently contains `295` tests, covering target detection and session protection, suppression and recovery (including PID reuse and crash wake-up), CPU topology and partitioning, profile storage format compatibility and unknown-version protection, game scanning and accelerator filtering, the boundaries of the launcher-learning mechanism, system audit thresholds, League column boundaries and UI rendering. A missing platform capability is recorded as `SKIP`, never as `PASS`.
