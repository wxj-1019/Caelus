...
[![Self-tests](https://img.shields.io/badge/self%2Dtests-253%20passing-%233DD68C)](https://github.com/wxj-1019/Caelus)
...
The built-in suite currently contains `253` tests, covering target detection and session protection, suppression and recovery (including PID reuse and crash wake-up), CPU topology and partitioning, profile storage format compatibility and unknown-version protection, game scanning and accelerator filtering, the boundaries of the launcher-learning mechanism, system audit thresholds, League column boundaries and UI rendering. A missing platform capability is recorded as `SKIP`, never as `PASS`.
