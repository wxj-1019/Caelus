...
[![自测](https://img.shields.io/badge/自测-295%20项%200%20失败-%233DD68C)](https://github.com/wxj-1019/Caelus)
...
| 🧪 **实测说话** | 内置 295 项自测 + 真机台架数据：压制收益、中断分布、NVIDIA 写入全部有数据支撑 |
...
- **开发专注**：编译期真后台压制 + 索引服务暂停 + 编译器提优；专注模式免打扰（通知静默 + 分心提醒，可选自动阻断）；IDE 家族（VS / VS Code / Rider / JetBrains / Cursor）自动提优，名录支持自定义扩展
- **日常优化**：浏览器 / Office / 会议家族活跃时常规档压制与提优；电池供电自动升档并即时感知；到点执行健康维护（着色器缓存清理 + 启动项基线审查）
- **开发服务守护**：清单内的本地服务（node、redis-server 等）不被后台压制，最后一个实例退出时提醒，可开启按原命令行自动拉起（连败熔断、稳定后重置预算，默认关闭）
- **专注统计**：专注时长按日历史（本地保存 60 天）+ 详情页近 7 日趋势与每日目标进度；编译次数与时长真实起止记账；分心应用当日计数与按名 Top、可选自动阻断，全部只存本机
- **日常统计**：日常家族掌权时长按日历史 + 详情页近 7 日趋势；维护中心显示下次自动维护倒计时
- 高优先级场景激活时低优先级场景**还原式挂起**，退出后瞬时补位；游戏接管同一共享效果（服务暂停、通知静默）时走占用权直通，不做"还原再重做"的无谓抖动；每个被动过的进程照旧记快照
...
dev.cmd test           rem 构建含自测的 WPF 版，跑完 295 项自测并输出结果摘要
...
<div align="center">
<img src="docs/overview-v15.png" width="49%" alt="概览">
<img src="docs/library-v15.png" width="49%" alt="游戏库">
<img src="docs/policy-v15.png" width="49%" alt="优化策略">
<img src="docs/graphics-v15.png" width="49%" alt="显卡">
<br>
<img src="docs/anticheat-v15.png" width="49%" alt="反作弊专项">
<img src="docs/devfocus-v15.png" width="49%" alt="开发专注">
<br>
<img src="docs/environment-v15.png" width="49%" alt="系统环境">
<img src="docs/whitelist-v15.png" width="49%" alt="白名单">
<br>
<img src="docs/audit-v15.png" width="49%" alt="系统体检">
<img src="docs/settings-v15.png" width="49%" alt="设置与恢复">
</div>
...
内置自测当前包含 **295 项**，覆盖目标识别与会话保护、压制与恢复（含 PID 复用、崩溃唤醒）、CPU 拓扑与分区、档案存储的格式兼容与未知版本保护、游戏扫描与加速器过滤、学习机制的触发边界、系统体检阈值、界面绘制与场景仲裁。机器缺少对应能力时会记录 `SKIP`，不会算作 `PASS`。
...
HAGS 与 VBS 的**实际生效**（需重启）、MMCSS、网络节流、Windows 更新暂停、Game DVR 关闭、电源覆盖、熄屏/睡眠守护、前台增益和真实反作弊兼容性目前没有端到端自动化测试，这些功能需要在目标机器上单独验证。注册表层面的开关往返（MPO 排查、HAGS 原值保持）、服务暂停/通知静默的场景交接（真实暂停与还原服务）已有自测覆盖。
...
- 实时监控页：三场景掌权状态瓦片、当前压制/提优进程清单（档位、来源、已压制时长）、最近 30 条动作流，2 秒自动刷新
