...
[![自测](https://img.shields.io/badge/自测-249%20项%200%20失败-%233DD68C)](https://github.com/wxj-1019/Caelus)
...
| 🧪 **实测说话** | 内置 249 项自测 + 真机台架数据：压制收益、中断分布、NVIDIA 写入全部有数据支撑 |
...
dev.cmd test           rem 构建含自测的 WPF 版，跑完 249 项自测并输出结果摘要
...
内置自测当前包含 **249 项**，覆盖目标识别与会话保护、压制与恢复（含 PID 复用、崩溃唤醒）、CPU 拓扑与分区、档案存储的格式兼容与未知版本保护、游戏扫描与加速器过滤、学习机制的触发边界、系统体检阈值、界面绘制与场景仲裁。机器缺少对应能力时会记录 `SKIP`，不会算作 `PASS`。
