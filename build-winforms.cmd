...
@rem file: FALLBACK build of the legacy WinForms UI (kept in the repo, not part
@rem of the main build). The main build.cmd produces the WPF host via MSBuild.
@rem Use this script only to produce a rollback Caelus.winforms.exe:
@rem   build-winforms.cmd Caelus.winforms.exe
@rem The same WinForms sources are compiled by dev.cmd test (the selftest
@rem build links src/Ui), so Core/UiShared API drift fails the self-test.
