...
日期：2026-09-11
状态：已实施 + 真机 E2E 验证（自测 276→287 全绿；规格偏差见 §11-§13）
...
自测 283→**286**（+3：预算重置、分心按名 Top8、阻断气球限频判定；后续 E2E 轮再 +1 至 287）。处置记录：
...
## 13. 真机 E2E 验证与发版（2026-09-12）

`scripts/e2e-devfocus.ps1`（一次提权会话）真机验证 **11 项断言全过**：

- 分心阻断：真实 notepad 启动即被自动关闭（优雅关闭路径）；计数/按名统计/TSV 当日列/运行日志四处落盘一致；二次命中静默再阻断。
- 服务自动拉起：Caelus 启动前已运行的 ping（快照入册）被杀后按原命令行重启，参数 `-t 127.0.0.1` 完整保留。

E2E 设计与推演过程中发现并修复三个真问题（均先于 E2E 运行落地）：

1. **守护服务与分心阻断互杀**：同名进程同时进两份清单且两开关全开时，「自动拉起 ↔ 专注阻断」形成拉起→关闭→再拉起死循环。修复：服务清单优先（`DecideDistractAction` 增加 isDevService 参数，命中即 None）。
2. **预存服务失明**：ProcNotify 纯 WMI 事件驱动，启动前已运行的服务没有 Started 事件，退出不提醒更不拉起。修复：`CaptureSnapshot` 把预存实例纳入跟踪。
3. **预算重置路径缺口**：nocmd 熔断后即使重新捕获到命令行也要等稳定 60 秒。修复：`StoreLaunch` 即清连败计数。

E2E 脚本侧的两个环境坑（记录备查）：Win11 商店版 notepad 走启动别名，`Start-Process -PassThru` 拿到的是毫秒级退出的存根进程，断言必须用产品信号（计数器增长 + notepad 全体消失）而非存根 PID 存活；`Caelus.log` 为无 BOM UTF-8，PS5.1 `Get-Content` 须显式 `-Encoding UTF8` 否则中文匹配恒失败。

发版：版本号 1.9.2 → **1.10.0**（src/wpf AssemblyInfo、App.cs、Caelus.iss、build-installer.cmd 五文件同步），ReleaseNotes 增补 1.10.0 条目，三语 README 补开发专注深化特性与自测计数 287。
