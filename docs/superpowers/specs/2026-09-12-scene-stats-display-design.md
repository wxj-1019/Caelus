...
## 13. 实机问题修复：编译位压制误伤媒体播放（2026-09-12）

**用户报告**：应用运行期间看视频卡顿。日志证据：开发专注因环境常驻编译进程掌权，压制 191 个后台进程，浏览器的 GPU/解码进程（无窗口）被降优先级 → 播放链路卡。

**修复**：压制豁免组合收敛为 `DevFocus.ComposeWhitelist`（两宿主共用单点改动）——游戏白名单 OR 守护服务 **OR 日常家族（DailyCatalog 名称+路径双校验）**。与日常场景「家族豁免压制」语义一致；代价是编译期前台媒体/办公应用少让少量 CPU（Eco 常规档）。

**测试**：豁免组合语义测试（家族/服务/游戏白名单透传 + 无关进程仍压制）。自测 292→293。

**遗留观察（不做）**：环境常驻编译类进程（MSBuild 驻留节点等）使「编译中」检测长期为真、开发专注长时间掌权。真实编译活跃度判定需要 CPU 采样，属独立课题；当前可用开关/白名单兜底。
