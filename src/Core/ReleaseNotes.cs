...
namespace CaelusApp
{
    ...
    internal static class ReleaseNotes
    {
        ...
        {
            new ReleaseNote("1.9.1", "2026-08-22", new[]
            {
                new[]{ "修复场景仲裁不灵敏：启动时初始全量扫描已运行的进程（如已开的 VS Code、浏览器），日常家族补算可见窗口，IDE 必须有可见窗口才激活场景（后台常驻的 IDE 辅助进程不再误掌权）。" },
                new[]{ "修复场景开关关闭不立即生效：开发/日常/电池开关现在立即退出仲裁器并还原副作用，不再出现「关闭后游戏退出时场景又恢复掌权」。" },
                new[]{ "修复 IDE / 浏览器提优崩溃自愈：提优快照持久化到 CrashGuard 日志，Caelus 崩溃后下次启动自动还原，目标进程不再停留在 AboveNormal + IO 3。" },
                new[]{ "修复卸载风险：安装器不再删除恢复快照注册表键（PrevSvcPaused/电源计划等），卸载改为优雅退出等待还原完成。" },
                new[]{ "修复深浅主题双入口冲突：旧「亮色主题」开关与新三态（深/浅/跟随系统）同步写 UiToneMode，重启后保持一致；消除三态切换的双重换肤。" },
                new[]{ "修复场景授权失败状态残留：Grant 抛异常时仲裁器回滚为无人掌权，不再显示「掌权中」但副作用未施加。" },
                new[]{ "修复 dev.cmd test 吞失败退出码：自测有 FAIL 时以非零退出码结束；README 英文/日文测试数同步为 229。" },
            }),
