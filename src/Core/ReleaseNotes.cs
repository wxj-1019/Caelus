...
namespace CaelusApp
{
    ...
    internal static class ReleaseNotes
    {
        ...
        {
            new ReleaseNote("1.10.0", "2026-09-12", new[]
            {
                new[]{ "开发专注深化四件套：IDE 名录开放自定义（与编译/日常名录同模式，设置页直接编辑）；专注时长按日历史（本地保存 60 天）+ 详情页近 7 日趋势柱图（今日高亮，分心/阻断计入当日）；分心应用命中统计（当日总数 + 按名 Top8）；开发服务退出自动拉起（WMI 捕获原命令行原参数重启，连败 5 次熔断、稳定 60 秒重置预算，默认关闭）。" },
                new[]{ "专注安全设计：分心阻断命中先优雅关闭、1 秒不退再结束进程；守护服务清单优先于分心阻断——同名进程不会被「自动拉起 ↔ 专注阻断」互相残杀成死循环；阻断气球 30 秒/名限频，被阻断的自启循环应用不再刷屏；Caelus 启动前已在运行的守护服务纳入跟踪（快照入册），退出提醒与自动拉起对预存实例同样生效。" },
                new[]{ "界面：夜空烟花 Hero 体系全页铺开（Aurora 双光晕、勋章图标、分组头图标令牌化）；截图探针支持视口高度与滚动偏移，长页可分段出全图；全矩阵三模式×明暗截图重摄。" },
                new[]{ "质量：内置自测 276 → 287 项全部通过；分心阻断与服务自动拉起完成真机端到端验证（真实进程关闭/重启、参数保留、计数与日志落盘，11 项断言全过）。" },
            }),
            new ReleaseNote("1.9.2", "2026-08-30", new[]
            {
                new[]{ "受保护游戏完整支持：内核反作弊（ACE/Vanguard 等）剥离句柄导致游戏完全不可见的问题修复——识别降级走 Toolhelp32+WMI 通道，检测/选举/自动入库/崩溃自愈全链可用；概览页如实显示「已被反作弊保护」而非整场谎报「正在提速」，对局报告注明本体未计入统计。" },
                new[]{ "场景引擎强化：场景交接移入专职泵线程，游戏启动/退出检测不再被编译期交接拖延；还原链逐步故障隔离（单步异常不再跳过后续还原）；电池插拔立即重扫压制档位；健康维护改为独立调度并在游戏进行中自动顺延。" },
                new[]{ "GPU 观测上线：概览页实时显示 GPU 温度（NVML 枚举全部显卡取最热，驱动故障 10 分钟冷却重建），被压频时附温度墙/功耗墙归因；AMD 显卡新增温度墙启发式采样；修复 AMD 快照按枚举序号记账在多卡/驱动更新后还原错卡的问题。" },
                new[]{ "游戏识别与扫描：Steam 软件类应用（Wallpaper Engine/Lossless Scaling 等 9 款）不再误入游戏库；Xbox 旧式安装路径（ModifiableWindowsApps）纳入扫描；游戏库扫描只扫固定磁盘并加 90 秒预算防假死；Epic/战网/商店协议快捷方式拖入时给出准确提示。" },
                new[]{ "界面与体验：托盘菜单改动即时回刷全部页面；环境页开关改后台执行不再冻结界面，完成后可选热重启设备立即生效；白名单批量拖入给出添加/跳过/失败计数；更新气泡可点击直达关于页；主窗口尺寸与位置记忆；引擎启动失败弹窗明示；修复体检页进度环切页后冻结。" },
                new[]{ "稳定性：渲染进度环在动画主题下每帧抛异常的渲染风暴修复（crash.log 279 连发）；场景提优崩溃快照采真实页面/GPU/亲和值（此前自愈会误写 very-low）；NVIDIA 驱动升级中途不再误删快照；AMD Anti-Lag 等快照改稳定显卡身份；LegacyPurge 升级迁移不再触碰活设置存储。" },
                new[]{ "其他：竞技电源档电池供电时 DC 侧自动转温和列；电源滑块覆盖电池档；反作弊名录补米哈游 HoYoKProtect/Xigncode3 等并支持自定义清单；自保护名单全压制通道统一短路省去无效重试；内置自测 249 → 253 项全部通过，UIA 交互套件实机 5/5。" },
            }),
