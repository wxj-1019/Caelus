        ...
        public static List<Entry> LoadBaseline(string path)
        {
            ...
            try
            {
                ...
                foreach (string line in File.ReadAllLines(path))
                {
                    ...
                    list.Add(new Entry(HealthEsc.Unesc(parts[0]), HealthEsc.Unesc(parts[1]), HealthEsc.Unesc(parts[2])));
                }
            }
            catch { }
            return list;
        ...
        public static void SaveBaseline(string path, List<Entry> entries)
        {
            try
            {
                ...
                foreach (Entry e in entries)
                    lines.Add(HealthEsc.Esc(e.Source) + "\t" + HealthEsc.Esc(e.Name) + "\t" + HealthEsc.Esc(e.Command));
